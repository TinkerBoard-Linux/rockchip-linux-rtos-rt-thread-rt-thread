...
#ifndef __RT_RKNPU_IOCTL_H
#define __RT_RKNPU_IOCTL_H
...
#define RKNPU_GET_DRV_VERSION_STRING(MAJOR, MINOR, PATCHLEVEL) \
        RKNPU_STR_HELPER(MAJOR) "." RKNPU_STR_HELPER(MINOR) "." RKNPU_STR_HELPER(PATCHLEVEL)
...
enum e_rknpu_mem_sync_mode
{
    ...
    RKNPU_MEM_SYNC_MASK = RKNPU_MEM_SYNC_TO_DEVICE | RKNPU_MEM_SYNC_FROM_DEVICE
};
...
#define RKNPU_CTL(nr, type) (nr)

#define RKNPU_IOC_MAGIC 'r'
#define RKNPU_IOW(nr, type) RKNPU_CTL(nr, sizeof(type))
#define RKNPU_IOR(nr, type) RKNPU_CTL(nr, sizeof(type))
#define RKNPU_IOWR(nr, type) RKNPU_CTL(nr, sizeof(type))
...
#define IOCTL_RKNPU_MEM_CREATE  RKNPU_IOWR(RKNPU_MEM_CREATE, struct rknpu_mem_create)
#define IOCTL_RKNPU_MEM_MAP RKNPU_IOWR(RKNPU_MEM_MAP, struct rknpu_mem_map)
#define IOCTL_RKNPU_MEM_DESTROY RKNPU_IOWR(RKNPU_MEM_DESTROY, struct rknpu_mem_destroy)
