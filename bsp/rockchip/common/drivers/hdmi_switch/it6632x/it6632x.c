...
rt_err_t SB_I2C_Read(rt_uint8_t u8Offset, rt_uint8_t u8ByteNo, rt_uint8_t *pu8Data)
{
    return it6632x_read_regs(u8Offset, u8ByteNo, pu8Data);
}

rt_err_t SB_I2C_Write(rt_uint8_t u8Offset, rt_uint8_t u8ByteNo, rt_uint8_t *pu8Data)
{
...
rt_err_t IT66322_send_cec_command(rt_uint8_t *ceccmd, rt_uint8_t len)
{
    if (len > 16 || len < 1)
    {
 ...
 */
void SB_CecUserCtl_Mute(void)
{
    it6632x_cec_cmd_t op = IT6632X_CEC_CTRL_VOL_MUTE;
 ...
 *choose ctrl local amp vol or send cec cmd to ctrl stb vol by hook implementation
 */
void SB_CecUserCtl_VolumeUp(void)
{
    it6632x_cec_cmd_t op = IT6632X_CEC_CTRL_VOL_UP;
    rt_kprintf("Volume+ \n");
    if (g_it6632x_dev->cec_vol_change_hook != RT_NULL)
        g_it6632x_dev->cec_vol_change_hook((void *)&op);
}
 ...
 *choose ctrl local amp vol or send cec cmd to ctrl stb vol by hook implementation
 */
void SB_CecUserCtl_VolumeDown(void)
{
    it6632x_cec_cmd_t op = IT6632X_CEC_CTRL_VOL_DOWN;
    rt_kprintf("Volume- \n");
    if (g_it6632x_dev->cec_vol_change_hook != RT_NULL)
        g_it6632x_dev->cec_vol_change_hook((void *)&op);
}

void SB_AudioIn_SPDIF(rt_uint32_t u32SbAdoInfo, rt_uint8_t u8SbInfoCA)
{
...
void SB_AudioIn_I2S(rt_uint32_t u32SbAdoInfo, rt_uint8_t u8SbInfoCA)
{
...
static void it6632x_audio_path_select(it6632x_audio_path_t ado_path)
{
    rt_uint8_t  u8I2cChg = 0;
    rt_uint8_t  u8Temp;
    SB_I2C_Read(I2C_SYS_CHANGE, 1, &u8I2cChg);
...
static void it6632x_arc_volume_get(struct it6632x_volume_info *info)
{
    rt_uint8_t  u8I2cChg = 0;
    rt_uint8_t  u8Temp;
    SB_I2C_Read(I2C_SYS_CHANGE, 1, &u8I2cChg);
...
static void it6632x_show_hdmi_decoderinfo(struct it6632x_ado_info *ado_info)
{
    ...
    if (ado_info->audio_type == IT6632X_AUDIO_TYPE_LPCM)
        rt_kprintf("LPCM");
    else if (ado_info->audio_type == IT6632X_AUDIO_TYPE_NLPCM)
        rt_kprintf("NLPCM");
    else if (ado_info->audio_type == IT6632X_AUDIO_TYPE_HBR)
        rt_kprintf("HBR");
    else if (ado_info->audio_type == IT6632X_AUDIO_TYPE_DSD)
        rt_kprintf("DSD");
...
static rt_err_t it6632x_register_audio_change_hook(void (*hook)(void *arg))
{
    g_it6632x_dev->audio_change_hook = hook;
    return RT_EOK;
}

static rt_err_t it6632x_unregister_audio_change_hook(void)
{
    g_it6632x_dev->audio_change_hook = RT_NULL;
    return RT_EOK;
}

static rt_err_t it6632x_register_cec_vol_change_hook(void (*hook)(void *arg))
{
    g_it6632x_dev->cec_vol_change_hook = hook;
    return RT_EOK;
}

static rt_err_t it6632x_unregister_cec_vol_change_change_hook(void)
{
...
static rt_err_t it6632x_unregister_rx_mute_hook(void)
{
...
static rt_err_t it6632x_control(rt_device_t dev, int cmd, void *args)
{
    ...
    switch (cmd)
    {
    case RT_IT6632X_CTRL_POWER_MODE_SET:
        it6632x_power_mode(*((rt_uint8_t *)args));
        break;
    ...
    case RT_IT6632X_CTRL_HDMI_GET_DECODERINFO:
        it6632x_get_hdmi_decoderinfo(&g_it6632x_dev->ado_info);
        break;
    case RT_IT6632X_CTRL_REGISTER_AUDIO_CHANGE_HOOK:
        it6632x_register_audio_change_hook(args);
        break;
    case RT_IT6632X_CTRL_UNREGISTER_AUDIO_CHANGE_HOOK:
        it6632x_unregister_audio_change_hook();
        break;
    case RT_IT6632X_CTRL_REGISTER_CEC_VOL_CHANGE_HOOK:
        it6632x_register_cec_vol_change_hook(args);
        break;
...
{
    struct audio_info *info = (struct audio_info *)arg;
...
static void it6632x_cec_vol_change_test_hook(void *arg)
{
    ...
    if (*(it6632x_cec_cmd_t *)arg == IT6632X_CEC_CTRL_VOL_MUTE)
        rt_kprintf("cec vol mute\n");
    else if (*(it6632x_cec_cmd_t *)arg == IT6632X_CEC_CTRL_VOL_UP)
        rt_kprintf("cec vol up\n");
    else if (*(it6632x_cec_cmd_t *)arg == IT6632X_CEC_CTRL_VOL_DOWN)
...
void it6632x(int argc, char **argv)
{
    ...
    struct it6632x_volume_info volume_info;
    if (argc < 2)
        goto out;
    ...
    if (argc == 2)
    {
        ...
        {
            rt_device_control(it6632x_dev, RT_IT6632X_CTRL_ARC_VOL_GET, (void *)&volume_info);
    ...
    else if (argc == 3)
    {
        ...
        else if (!strcmp(argv[1], "hookregister"))
        {
            ...
            if (para == 1)
            {
                rt_device_control(it6632x_dev, RT_IT6632X_CTRL_REGISTER_AUDIO_CHANGE_HOOK, (void *)&it6632x_audio_change_test_hook);
            }
            else if (para == 2)
            {
                rt_device_control(it6632x_dev, RT_IT6632X_CTRL_REGISTER_CEC_VOL_CHANGE_HOOK, (void *)&it6632x_cec_vol_change_test_hook);
            }
            else if (para == 3)
            {
        ...
        else if (!strcmp(argv[1], "hookunregister"))
        {
            ...
            if (para == 1)
            {
                rt_device_control(it6632x_dev, RT_IT6632X_CTRL_UNREGISTER_AUDIO_CHANGE_HOOK, RT_NULL);
            }
            else if (para == 2)
            {
                rt_device_control(it6632x_dev, RT_IT6632X_CTRL_UNREGISTER_CEC_VOL_CHANGE_HOOK, RT_NULL);
            }
            else if (para == 3)
            {
                rt_device_control(it6632x_dev, RT_IT6632X_CTRL_UNREGISTER_RX_MUTE_HOOK, RT_NULL);
            }
            else
            {
                rt_kprintf("wrong usage, check your param \n", IT6632X_DEVICE_NAME);
    ...
    else if (argc == 4)
    {
        if (!strcmp(argv[1], "arcvolset"))
        {
            ...
            rt_device_control(it6632x_dev, RT_IT6632X_CTRL_ARC_VOL_SET, (void *)&volume_info);
        }
    }
    else if (argc > 4)
    {
